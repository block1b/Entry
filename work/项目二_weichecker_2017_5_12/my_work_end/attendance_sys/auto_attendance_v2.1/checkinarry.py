...
class CheckInArray(object):
    ...
    def remove_arry_same_class(self, now_cii, cii_arry):
        ...
        flag = 1
        for one_cii in cii_arry:
            if list(set(one_cii.classnames) & set(now_cii.classnames)):  # 人员冲突
    ...
    def update_arry(self, now_cii, cii_arry):
        # 队列更新步骤
        # 2.查看学生是否已经在考勤队列？
        if self.same_classnames(now_cii, cii_arry):
            ...
            if self.same_section(now_cii, cii_arry):
                ...
                # 踢队内学生冲突的对象
                new_hand = self.remove_arry_same_class(now_cii, cii_arry)
                cii_arry.append(now_cii)  # 该教师进入队列
                # 若队首被更新 判定方法待定
                if new_hand == 0:  # 队首被更新
                    self.tim.t.cancel()
                    timedev = self.ini.get_window_long() - (self.tim.get_time_sub(cii_arry[0].starttime, now_cii.starttime))
