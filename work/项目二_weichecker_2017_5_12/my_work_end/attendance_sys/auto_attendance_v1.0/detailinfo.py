...
class DetailInfo(BaseImport):
    ...
    def set_detail_result(self, detail_line, starttime):
        if detail_line["CheckinResult"] == "man":
            pass
        else:
            if detail_line["IsSucc"] == "False":
                detail_line["CheckinResult"] = "qq"
            else:
                # 学生提交时间晚于教师发起时间起0-5，5-10，10+
                time_sub = self.get_time_sub(starttime, detail_line["CheckinTime"])
                if time_sub >=0 and time_sub <= 300:
                    detail_line["CheckinResult"] = "dq"
                if time_sub >300 and time_sub <= 600:
                    detail_line["CheckinResult"] = "cd"
                if time_sub > 600:
                    detail_line["CheckinResult"] = "qq"
